...
//
//            System.out.printf("Captcha: %d %s %d = ?%n", a, operator, b);
...
//        }
//
//// ASCII Art Captcha section
//        while(true) {
//            String[] captchaOptions = {"cat", "dog", "fish", "rabbit", "bird"};
//            List<String> selectedCaptchas = new ArrayList<>();
//
//            // Ensure we get 3 unique random animals
//            while (selectedCaptchas.size() < 3) {
//                String animal = captchaOptions[random.nextInt(captchaOptions.length)];
//                if (!selectedCaptchas.contains(animal)) {
//                    selectedCaptchas.add(animal);
//                }
//            }
//
//            for (String selectedCaptcha : selectedCaptchas) {
...
//            }
//            boolean a = true;
//            System.out.println("Please type the names of the animals shown in the captcha sequentially:");
//            for (String selectedCaptcha : selectedCaptchas) {
//                String userAnswer = sc.nextLine().trim().toLowerCase();
//                if (!userAnswer.equals(selectedCaptcha)) {
//                    a = false;
//                }
//            }
//
...
//        }
    ...
    public void SelectCharacter(boolean isWinnerTakesAll,int Bet){
    ...
    private void GameOn(boolean isWinnerTakesAll,int Bet){
        ...
        if (arena.WinnerIsHost){
            ...
            System.out.println("Winner earned " + arena.HostHP*10 + " coins & " + arena.HostHP*5 + " XP");
            SecondUser.AddXP(250 - arena.HostHP*5);
            System.out.println("Loser earned " + (250 - arena.HostHP)*5 + " XP");
            LocalDateTime localDateTime = LocalDateTime.now();
            loggedInUser.gameHistory.add(new GameHistory(localDateTime,"Won",
                    SecondUser.nickname , SecondUser.Level,"Earned " + arena.HostHP*10 + " coins & " + arena.HostHP*5 + " XP"));
            SecondUser.gameHistory.add(new GameHistory(localDateTime,"Lost",
                    loggedInUser.nickname , loggedInUser.Level,"Earned " + (250 - arena.HostHP)*5 + " XP"));
            if (isWinnerTakesAll) {
                loggedInUser.coins += Bet;
                SecondUser.coins -= Bet;
                System.out.println("Host won " + 2*Bet + " coins");
            }
        }
        else {
            System.out.println("Guest!");
            SecondUser.AddXP(arena.GuestHP*5);
            SecondUser.coins += arena.GuestHP*10;
            System.out.println("Winner earned " + arena.GuestHP*10 + " coins & " + arena.GuestHP*5 + " XP");
            loggedInUser.AddXP(250 - arena.GuestHP*5);
            System.out.println("Loser earned " + (250 - arena.GuestHP)*5 + " XP");
            LocalDateTime localDateTime = LocalDateTime.now();
            SecondUser.gameHistory.add(new GameHistory(localDateTime,"Won",
                    loggedInUser.nickname , loggedInUser.Level,"Earned " + arena.GuestHP*10 + " coins & " + arena.GuestHP*5 + " XP"));
            loggedInUser.gameHistory.add(new GameHistory(localDateTime,"Lost",
                    SecondUser.nickname , SecondUser.Level,"Earned " + (250 - arena.GuestHP)*5 + "XP"));
            if (isWinnerTakesAll) {
                SecondUser.coins += Bet;
                loggedInUser.coins -= Bet;
                System.out.println("Guest won " + 2*Bet + " coins");
            }
        }
        MainMenu();
    }
