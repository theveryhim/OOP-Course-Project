...
public class Arena {
    ...
    public void PlayerTurn(boolean isHost){
        ...
        if (isHost){
            ...
            while (!Played){
                ...
                else if (command.matches("Placing card n (?<n>[1-6]) in tile (?<t>([0-9]|1[0-9]|20|21)$)")){
                    Matcher matcher = getCommandMatcher(command,
                            "Placing card n (?<n>[1-6]) in tile (?<t>([0-9]|1[0-9]|20|21)$)");
                    matcher.find();
                    int n = Integer.parseInt(matcher.group("n"));
    ...
    public void Display(boolean isHost){
        ...
        if (HostBrokenTile!=-1)
            ...
            System.out.println("Host Broken Tile: " + HostBrokenTile);
        if (GuestBrokenTile !=-1)
            System.out.println("Guest Broken Tile: " + (GuestBrokenTile + 1));
        else
            System.out.println("Guest Broken Tile: " + GuestBrokenTile);
    ...
    public boolean PlayCard(int n,boolean isHost,Card card){
        ...
        if (isHost) {
            ...
            switch (card.Ability) {
                ...
                case 7:/*Card Robber*/{
                    if (HostDeck.size() < 6){
                        int r = random.nextInt(GuestDeck.size());
            ...
            switch (card.Ability) {
                ...
                case 7:/*Card Robber*/{
                    if (GuestDeck.size() < 6){
                        int r = random.nextInt(HostDeck.size());
    ...
    public void TimeLine(){
        ...
        for (int i = 0; i < 21 & (HostHP > 0) & (GuestHP > 0); i++){
            ...
            if (HostTiles[i].AttackDefence>300)
                System.out.println("Host card AttackDefence: Infinite");
            else
                System.out.println("Host card AttackDefence: " + HostTiles[i].AttackDefence);
            if (GuestTiles[i].AttackDefence>300)
                System.out.println("Guest card AttackDefence: Infinite");
            else
                System.out.println("Guest card AttackDefence: " + GuestTiles[i].AttackDefence);
            System.out.println("Host card Damage: " + HostTiles[i].PlayerDamage);
            System.out.println("Guest card Damage: " + GuestTiles[i].PlayerDamage);
            if (!HostTiles[i].IsDefended)
